...
describe('SQL Generation', () => {
  ...
  it('Test for everyRefreshKeySql', async () => {
    ...
    expect(query.everyRefreshKeySql({ every: '0 * * * *', timezone }))
      .toEqual([`FLOOR((${utcOffset} + 0 + EXTRACT(EPOCH FROM NOW())) / 3600)`, false]);

    expect(query.everyRefreshKeySql({ every: '0 10 * * *', timezone }))
      .toEqual([`FLOOR((${utcOffset} + 36000 + EXTRACT(EPOCH FROM NOW())) / 86400)`, false]);

    // Additional syntax with seconds (seconds minutes hours day month dow)
    expect(query.everyRefreshKeySql({ every: '0 * * * * *', timezone, }))
      .toEqual([`FLOOR((${utcOffset} + 0 + EXTRACT(EPOCH FROM NOW())) / 60)`, false]);

    expect(query.everyRefreshKeySql({ every: '0 * * * *', timezone }))
      .toEqual([`FLOOR((${utcOffset} + 0 + EXTRACT(EPOCH FROM NOW())) / 3600)`, false]);

    expect(query.everyRefreshKeySql({ every: '30 * * * *', timezone }))
      .toEqual([`FLOOR((${utcOffset} + 1800 + EXTRACT(EPOCH FROM NOW())) / 3600)`, false]);

    expect(query.everyRefreshKeySql({ every: '30 5 * * 5', timezone }))
      .toEqual([`FLOOR((${utcOffset} + 365400 + EXTRACT(EPOCH FROM NOW())) / 604800)`, false]);

    for (let i = 1; i < 59; i++) {
      expect(query.everyRefreshKeySql({ every: `${i} * * * *`, timezone }))
        .toEqual([`FLOOR((${utcOffset} + ${i * 60} + EXTRACT(EPOCH FROM NOW())) / ${1 * 60 * 60})`, false]);
    }
